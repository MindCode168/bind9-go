...
'''.strip()+'\n')
    ...
    report = {'schema_version':1,'title':'BIND 与 Go DNS 服务器性能测试报告',
        ...
        'methodology':{'tool':'dnsperf 2.14.0','duration_seconds':DURATION,'warmup_seconds':WARMUP,
                       ...
                       'repetitions':REPEATS,'rates_qps':RATE,'zone_records':10003,'queried_a_records':10000,
        ...
        'limitations':[
            ...
            'CPU 为区间进程时间差，100% 表示一个逻辑核；观察窗口含客户端启动和退出等待，略长于 dnsperf 统计窗口。RSS 为每 100ms 采样所得峰值。',
